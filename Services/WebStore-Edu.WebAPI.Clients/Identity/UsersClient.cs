...
namespace WebStore_Edu.WebAPI.Clients.Identity
{
    public class UsersClient : BaseClient, IUserClient
    {
        ...
        {
            var response = await PostAsync($"{Address}/SetPasswordHash", new UserDTO { User = user, PasswordHash = hash }, cancel)
               .ConfigureAwait(false);
        ...
        {
            await PostAsync($"{Address}/AddLogin", new UserDTO { User = user, LoginInfo = login }, cancel).ConfigureAwait(false);
        }
        ...
        public async Task SetLockoutEndDateAsync(User user, DateTimeOffset? EndDate, CancellationToken cancel)
        {
            var response = await PostAsync(
                    ...
                    new UserDTO { User = user, LockoutEnd = EndDate },
                    cancel)
               .ConfigureAwait(false);
        ...
        public async Task AddClaimsAsync(User user, IEnumerable<Claim> claims, CancellationToken cancel)
        {
            await PostAsync(
                    $"{Address}/AddClaims",
                    new UserDTO { User = user, Claims = claims },
                    cancel)
               .ConfigureAwait(false);
        }
        ...
        public async Task ReplaceClaimAsync(User user, Claim OldClaim, Claim NewClaim, CancellationToken cancel)
        {
            await PostAsync(
                    ...
                    new UserDTO { User = user, Claims = new [] { OldClaim, NewClaim} },
                    cancel)
               .ConfigureAwait(false);
        }
